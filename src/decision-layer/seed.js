...
export async function seedDecisionLayer(at = '2026-10-02T00:00:00.000Z') {
  ...
  for (const [id, externalCustomerId, name, jurisdiction, balance, oldestDebtDays, hardshipStatus, currentPlan, bestOfferOptOut, sensitiveCustomer] of CUSTOMERS) {
    ...
    const sources = Object.entries(state).map(([field, value]) => ({ field, value, source: field === 'currentPlan' ? 'PRICING' : 'BILLING', lastUpdated: at, confidence: 1, extractionMethod: 'SYNTHETIC_SEED' }));
    rows.push(await upsertCustomer({ id, externalCustomerId, name, jurisdiction, state, sources }));
  }
  return rows;
}
